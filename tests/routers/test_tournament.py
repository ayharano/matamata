...
from tests.utils import register_match_result_util, start_tournament_util
...
REGISTER_COMPETITOR_IN_TOURNAMENT_URL_TEMPLATE = (
    BASE_URL + "/{tournament_uuid}/competitor"
)
...
def test_409_for_started_tournament_during_register_competitor_in_tournament(
...
def test_404_for_missing_competitor_during_register_competitor_in_tournament(
    client, tournament
):
    response = client.post(
        REGISTER_COMPETITOR_IN_TOURNAMENT_URL_TEMPLATE.format(
...
def test_409_for_already_registered_competitor_during_register_competitor_in_tournament(
    client, session, tournament, competitor
):
...
LIST_MATCHES_FOR_COMPETITOR_IN_TOURNAMENT_URL_TEMPLATE = (
    BASE_URL + "/{tournament_uuid}/competitor/{competitor_uuid}"
)
...
def test_404_for_missing_tournament_during_list_matches_for_competitor_in_tournament(
    client, competitor
):
    response = client.get(
        LIST_MATCHES_FOR_COMPETITOR_IN_TOURNAMENT_URL_TEMPLATE.format(
...
def test_404_for_missing_competitor_during_list_matches_for_competitor_in_tournament(
    client, tournament
):
    response = client.get(
        LIST_MATCHES_FOR_COMPETITOR_IN_TOURNAMENT_URL_TEMPLATE.format(
...
def test_201_for_start_tournament_for_one_competitor(
    ...
    # as UUID is dynamically generated, we will extract it for the assertion
    matches_uuid = [match_data["uuid"] for match_data in response_json["matches"]]
...
def test_409_for_started_tournament_during_start_tournament(
    client, session, tournament, competitor
):
    # Already started tournament
    tournament.matches_creation = datetime(year=2024, month=1, day=1)
...
def test_200_for_list_tournament_matches_for_one_competitor(
    session, client, tournament, competitor
):
    tournament.competitors.append(competitor)
    session.add(tournament)
    ...
    # as UUID is dynamically generated, we will extract it for the assertion
    past_matches_uuid = [match_data["uuid"] for match_data in response_json["past"]]
...
def test_200_list_tournament_matches_for_five_competitors(
    ...
    upcoming_matches_uuid = [
        match_data["uuid"] for match_data in response_json["upcoming"]
    ]
...
def test_422_for_missing_competitor_during_list_tournament_matches(
    session, client, tournament, competitor
):
    tournament.competitors.append(competitor)
    session.add(tournament)
...
def test_200_for_get_tournament_top4_for_one_competitor(
    session, client, tournament, competitor
):
    tournament.competitors.append(competitor)
    session.add(tournament)
...
def test_200_for_get_tournament_top4_for_four_competitors(
    ...
    client.post(
        REGISTER_MATCH_RESULT_URL_TEMPLATE.format(
            match_uuid=json_third_place_match["uuid"]
        ),
        json={
...
def test_422_for_missing_competitor_during_get_tournament_top4(
    session, client, tournament, competitor
):
    tournament.competitors.append(competitor)
    session.add(tournament)
